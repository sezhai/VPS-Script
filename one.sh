...
# 全局设置（不增加功能，仅提高稳健性）
# -----------------------------------------------------------------------------
# 若系统存在 sudo 则使用，否则直接本地执行，避免 “sudo: command not found”
...
view_vps_info() {
  ...
  echo -e "\e[1;34mDNS地址:\e[0m \e[32m$(grep -E '^nameserver' /etc/resolv.conf | awk '{print $2}' | xargs | sed 's/ /, /g')\e[0m"
    ...
    case "$root_choice" in
      ...
      2)
        ...
        if [ ! -e "$file_path" ]; then
          ...
          chmod 755 "$file_path" && echo -e "\e[32m'$file_path' 权限已设置为 755！\e[0m" || echo -e "\e[31m设置权限失败。\e[0m"
        fi
        read -n 1 -s -r -p "按任意键返回..."
        echo
        ;;
        ...
        while true; do
          ...
          for choice in $choices; do
            if [[ "$choice" =~ ^[0-9]+$ ]] && (( choice >= 1 && choice <= ${#files[@]} )); then
              ...
              if [[ "$confirm" == "y" ]]; then
                if [[ -d "$file" ]]; then
                  rm -rf "$file" && echo -e "\e[32m目录已删除: $file\e[0m"
                else
                  rm -f "$file" && echo -e "\e[32m文件已删除: $file\e[0m"
                fi
              else
                echo "取消删除 $file。"
              fi
        ...
        while true; do
          ...
          if [[ "$pid" =~ ^[0-9]+$ ]]; then
            if kill "$pid"; then
              ...
              echo -e "\e[31m进程 $pid 无法正常关闭 (SIGTERM)，需要强制关闭 (SIGKILL)？ (y/n)\e[0m"
              read -p "请选择 (y/n): " choice
              if [[ "$choice" == "y" ]]; then
                kill -9 "$pid" && echo -e "\e[32m进程 $pid 已被强制关闭！\e[0m" || echo -e "\e[31m进程 $pid 强制关闭失败。\e[0m"
              else
                echo "取消强制关闭"
              fi
            fi
      ...
      6)
        if command -v ss &>/dev/null; then
          ...
          ss -tulnp | awk 'NR>1 {
            split($5, a, ":"); split($7, b, ",");
            gsub(/[()]/, "", b[1]); gsub(/pid=/, "", b[2]);
            gsub(/users:/, "", b[1]); gsub(/"/, "", b[1]);
      ...
      7)
        while true; do
          ...
          case "$protocol_choice" in
            1) protocol="tcp" ;;
            2) protocol="udp" ;;
            *) echo -e "\e[31m无效的选择，请输入1或2。\e[0m"; continue ;;
          esac
          read -p "请输入端口号: " port
          if ! [[ "$port" =~ ^[0-9]+$ ]] || [ "$port" -lt 1 ] || [ "$port" -gt 65535 ]; then
            echo -e "\e[31m无效的端口号，请输入1到65535之间的数字。\e[0m"; continue
          fi
    ...
    case "$opt_choice" in
      ...
      2)
        ...
        case "$action" in
          ...
          2) if $SUDO apt update && $SUDO apt remove -y sudo; then echo -e "\e[32msudo 卸载完成！\e[0m"; else echo -e "\e[31msudo 卸载失败！\e[0m"; fi ;;
          "") ;;
          *) echo -e "\e[31m无效选项，请重新输入。\e[0m" ;;
        esac
        read -n 1 -s -r -p "按任意键返回..."
      ...
      9)
        ...
        read -p "请选择操作 (默认回车退出)：" action
        case "$action" in
          1) if curl -sSL https://get.docker.com/ | sh; then echo -e "\e[32mdocker 安装完成！\e[0m"; else echo -e "\e[31mdocker 安装失败！\e[0m"; fi ;;
    ...
    case "$cert_choice" in
      1)
        ...
        if ! command -v crontab &>/dev/null; then
          echo "正在安装 cron..."
          $SUDO apt install -y cron && echo -e "\e[32mcron 安装完成！\e[0m" || echo -e "\e[31mcron 安装失败！\e[0m"
        fi
        if ! command -v socat &>/dev/null; then
          echo "正在安装 socat..."
          $SUDO apt install -y socat && echo -e "\e[32msocat 安装完成！\e[0m" || echo -e "\e[31msocat 安装失败！\e[0m"
        fi
      ...
      2)
        while true; do
          ...
          if ~/.acme.sh/acme.sh --issue --standalone -d "$domain"; then
            ...
            echo -e "\e[31m证书申请失败，请检查域名并重试。\e[0m"
          fi
          break
        done
        read -n 1 -s -r -p "按任意键返回..."
      ...
      3)
        ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt && \
          ...
          echo -e "\e[31m切换失败，请检查acme.sh及网络。\e[0m"
        read -n 1 -s -r -p "按任意键返回..."
        echo
        ;;
      4)
  ...
  while true; do
    ...
    case "$xray_choice" in
      1)
        ...
        ;;
      2)
        echo -e "\e[33m提示：将UUID填入配置文件。若已通过“申请证书”安装证书，证书路径可不改。\e[0m"
        read -n 1 -s -r -p "按任意键继续..."
        command -v nano >/dev/null 2>&1 || { $SUDO apt update >/dev/null 2>&1 && $SUDO apt install -y nano >/dev/null 2>&1; }
        command -v nano >/dev/null 2>&1 || { echo -e "\e[31m错误：无法安装或找到 nano！\e[0m"; return; }
      ...
      3)
        ...
        get_public_ip() {
          ...
          if [[ -n "$ipv4" ]]; then echo "$ipv4"; else curl -s -6 https://api64.ipify.org || echo "127.0.0.1"; fi
        }
        while true; do
          ...
          if [[ -z "$CERT_PATH" ]]; then
            echo -e "\e[31m未能找到证书路径。\e[0m"; break
          fi
...
# 安装VLESS-TCP-REALITY（修复：稳健抓取 x25519 私钥与公钥）
    ...
    case "$xray_choice" in
      1)
        if bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install && \
          ...
          # === 关键修正开始：稳健获取 PRIVATE_KEY / PUBLIC_KEY ===
          keys="$(xray x25519 2>&1 || true)"
          # 先尝试 “Private key: XXX / Public key: XXX” 形态
          PRIVATE_KEY="$(printf '%s\n' "$keys" | awk -F': ' '/^[[:space:]]*Private[[:space:]]*key[[:space:]]*:/{print $2; exit}')"
          PUBLIC_KEY="$(printf '%s\n' "$keys" | awk -F': ' '/^[[:space:]]*Public[[:space:]]*key[[:space:]]*:/{print $2; exit}')"
          # 兼容 “PrivateKey XXX / PublicKey XXX”（无冒号）
          if [ -z "$PRIVATE_KEY" ] || [ -z "$PUBLIC_KEY" ]; then
            PRIVATE_KEY="$(printf '%s\n' "$keys" | awk '/^[[:space:]]*PrivateKey[[:space:]]+/{print $2; exit}')"
            PUBLIC_KEY="$(printf '%s\n' "$keys" | awk '/^[[:space:]]*PublicKey[[:space:]]+/{print $2; exit}')"
          fi
        ...
        ;;
      2)
        echo -e "\e[33m提示：将UUID、目标网站及私钥填入配置文件中，ShortIds为可选。\e[0m"
        read -n 1 -s -r -p "按任意键继续..."
        command -v nano >/dev/null 2>&1 || { $SUDO apt update >/dev/null 2>&1 && $SUDO apt install -y nano >/dev/null 2>&1; }
        command -v nano >/dev/null 2>&1 || { echo -e "\e[31m错误：无法安装或找到 nano！\e[0m"; return; }
        $SUDO nano /usr/local/etc/xray/config.json
      ...
      3)
        ...
        extract_server_name() {
          grep -A 5 '"serverNames"' "$CONFIG_PATH" | grep -o '"[^"]*"' | head -n 2 | tail -n 1 | sed 's/"//g'
        }
        extract_list_field() {
          local list_parent=$1; local list_field=$2
    ...
    case "$hysteria_choice" in
      ...
      3)
        ...
        get_ip_address() {
          ...
          if [[ -n "$ip" && "$ip" =~ ^[0-9]{1,3}(\.[0-9]{1,3}){3}$ ]]; then echo "$ip"; return 0; fi
          ip=$(curl -6 -s --connect-timeout 5 https://ifconfig.me 2>/dev/null)
          if [[ -n "$ip" && "$ip" =~ ^[0-9a-fA-F:]+$ ]]; then echo "[$ip]"; return 0; fi
          return 1
        }
        if [ ! -f "$config_file" ]; then
          echo -e "\e[31m未能找到配置文件。\e[0m"; return
        fi
        while true; do
          ...
          if [ -z "$domain" ]; then
            ...
            [ -z "$cert_path" ] || [ ! -f "$cert_path" ] && echo -e "\e[31m没有找到域名或证书。\e[0m"
            domain=$(get_domain_from_cert "$cert_path")
            [ -z "$domain" ] && echo -e "\e[31m从证书中提取域名失败。\e[0m"
          fi
          ip=$(get_ip_address)
          [ -z "$ip" ] && echo -e "\e[31m无法获取IP地址，请检查网络连接。\e[0m"
      ...
      4)
        ...
        read -p "请输入起始端口号 (Enter 默认 60000): " start_port
        [[ -z "$start_port" ]] && start_port="$default_start_port"
        [[ "$start_port" =~ ^[0-9]+$ && "$start_port" -ge 1 && "$start_port" -le 65535 ]] || { echo -e "\e[31m起始端口无效, 使用默认 60000\e[0m"; start_port="$default_start_port"; }
        read -p "请输入结束端口号 (Enter 默认 65535): " end_port
        [[ -z "$end_port" ]] && end_port="$default_end_port"
        [[ "$end_port" =~ ^[0-9]+$ && "$end_port" -ge 1 && "$end_port" -le 65535 && "$end_port" -ge "$start_port" ]] || { echo -e "\e[31m结束端口无效，使用默认 65535\e[0m"; end_port="$default_end_port"; }
        mapfile -t interfaces < <(ip -o link | awk -F': ' '$2 != "lo" {print $2}')
        [[ ${#interfaces[@]} -eq 0 ]] && { echo -e "\e[31m未找到网络接口，无法执行 iptables。\e[0m"; return; }
  ...
  while true; do
    ...
    case "$singbox_choice" in
      ...
      2)
        ...
        read -n 1 -s -r -p "按任意键继续..."
        command -v nano >/dev/null 2>&1 || { $SUDO apt update >/dev/null 2>&1 && $SUDO apt install -y nano >/dev/null 2>&1; }
        command -v nano >/dev/null 2>&1 || { echo -e "\e[31m无法安装或找到 nano。\e[0m"; return; }
      ...
      3)
        ...
        get_ip() {
          ...
          if [[ -n "$ipv4" && "$ipv4" =~ ^[0-9]{1,3}(\.[0-9]{1,3}){3}$ ]]; then echo "$ipv4"; return 0; fi
          ...
          if [[ -n "$ipv6" && "$ipv6" =~ ^[0-9a-fA-F:]+$ ]]; then echo "$ipv6"; return 0; fi
        ...
        ip=$(get_ip)
        if [[ "$ip" =~ : ]] && [[ "$ip" != "127.0.0.1" ]]; then ip_for_url="[$ip]"; else ip_for_url="$ip"; fi
  ...
  while true; do
    ...
    case "$panel_choice" in
      ...
      4)
        ...
        ;;
      5)
        if $SUDO systemctl stop 1panel && $SUDO 1pctl uninstall && \
