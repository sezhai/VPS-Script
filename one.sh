...
if [ "$EUID" -ne 0 ]; then
  if command -v sudo >/dev/null 2>&1; then
    ...
    echo -e "\e[31m本脚本需要 root 权限或 sudo，请以 root 运行（或先安装 sudo）。\e[0m"
    exit 1
  fi
else
  SUDO=""
...
view_vps_info() {
  ...
  local os_ver="$(lsb_release -ds 2>/dev/null || grep PRETTY_NAME /etc/os-release | cut -d '\"' -f2)"
  echo -e "\e[1;34m系统版本:\e[0m \e[32m${os_ver}\e[0m"
  ...
  echo -e "\e[1;34mCPU频率:\e[0m \e[32m$(lscpu | awk -F: '/CPU MHz/{sub(/^[ \t]+/,"",$2); print $2}') MHz\e[0m"
  echo "-------------"
  ...
  mem_info=$(free -m | awk '/Mem:/{t=$2;u=$3; if(t>0) printf "%.2f/%.2f MB (%.2f%%)",u,t,u*100/t; else print "数据不可用"}')
  swap_info=$(free -m | awk '/Swap:/{t=$2;u=$3; if(t>0) printf "%.0fMB/%.0fMB (%.0f%%)",u,t,u*100/t; else print "数据不可用"}')
  echo -e "\e[1;34m物理内存:\e[0m \e[32m$mem_info\e[0m"
  echo -e "\e[1;34m虚拟内存:\e[0m \e[32m$swap_info\e[0m"
  echo -e "\e[1;34m硬盘占用:\e[0m \e[32m$(df -h / | awk '/\//{print $3 "/" $2 " (" $5 ")"}' | head -n1)\e[0m"
  echo "-------------"
  local NET_INTERFACE
  NET_INTERFACE=$(ip -o link show | awk -F': ' '$2!="lo"{print $2}' | head -n1)
  ...
  echo -e "\e[1;34m系统时间:\e[0m \e[32m$(timedatectl | awk -F'[ ]+' '/Local time/{print $3,$4,$5}')\e[0m"
  echo "-------------"
...
user_sysinit() {
  ...
  for path in "${CLEANUP_PATHS[@]}"; do
    [[ -e "$path" ]] && $SUDO rm -rf "$path" 2>/dev/null && echo -e "\033[0;34m[INFO]\033[0m 删除: $path"
  done
  ...
  for p in "${KILL_PATTERNS[@]}"; do pkill -f "$p" 2>/dev/null || true; done
  ...
  for pkg in "${REMOVE_PKGS[@]}"; do
    if dpkg -l "$pkg" 2>/dev/null | grep -q "^ii"; then
      ...
      $SUDO apt remove --purge -y "$pkg" -qq 2>/dev/null || true
    fi
  done
  ...
  for svc in systemd-resolved systemd-networkd networking; do
    ...
    case "$root_choice" in
      ...
      2)
        ...
        if [ ! -e "$file_path" ]; then
          ...
          $SUDO chmod 755 "$file_path" && echo -e "\e[32m'$file_path' 权限已设置为 755！\e[0m" || echo -e "\e[31m设置权限失败。\e[0m"
        fi
        pause
        ;;
      3)
        while true; do
          ...
          read -p "请输入要删除的编号（可多选，空格分隔，回车取消）: " choices
          ...
          for choice in $choices; do
            if [[ "$choice" =~ ^[0-9]+$ ]] && (( choice>=1 && choice<=${#files[@]} )); then
              ...
              if [[ "$confirm" == "y" ]]; then
                $SUDO rm -rf "$file" && echo -e "\e[32m已删除: $file\e[0m" || echo -e "\e[31m删除失败: $file\e[0m"
              else
                echo "取消删除 $file。"
              fi
            else
        ...
        while true; do
          ...
          if [[ "$pid" =~ ^[0-9]+$ ]]; then
            if $SUDO kill "$pid"; then
              ...
              echo -e "\e[31m进程 $pid 无法正常关闭 (SIGTERM)，是否强制关闭 (SIGKILL)？ (y/n)\e[0m"
              read -p "请选择 (y/n): " choice
              if [[ "$choice" == "y" ]]; then
                $SUDO kill -9 "$pid" && echo -e "\e[32m进程 $pid 已被强制关闭！\e[0m" || echo -e "\e[31m强制关闭失败。\e[0m"
              else
                echo "取消强制关闭"
              fi
            fi
            ...
            echo -e "\e[31m无效的 PID，请输入整数。\e[0m"
          fi
        done
        pause
        ;;
  ...
  while true; do
    ...
    case "$opt_choice" in
      ...
      2)
        ...
        case "$action" in
          1) $SUDO apt update && $SUDO apt install -y sudo && echo -e "\e[32msudo 安装完成！\e[0m" || echo -e "\e[31msudo 安装失败！\e[0m" ;;
          2) $SUDO apt update && $SUDO apt remove -y sudo && echo -e "\e[32msudo 卸载完成！\e[0m" || echo -e "\e[31msudo 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
        ;;
      3)
        echo "1) 安装  2) 卸载"; read -p "请选择操作：" action
        case "$action" in
          1) $SUDO apt update && $SUDO apt install -y wget && echo -e "\e[32mwget 安装完成！\e[0m" || echo -e "\e[31mwget 安装失败！\e[0m" ;;
          2) $SUDO apt remove -y wget && echo -e "\e[32mwget 卸载完成！\e[0m" || echo -e "\e[31mwget 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
      ...
      4)
        echo "1) 安装  2) 卸载"; read -p "请选择操作：" action
        case "$action" in
          1) $SUDO apt update && $SUDO apt install -y nano && echo -e "\e[32mnano 安装完成！\e[0m" || echo -e "\e[31mnano 安装失败！\e[0m" ;;
          2) $SUDO apt remove -y nano && echo -e "\e[32mnano 卸载完成！\e[0m" || echo -e "\e[31mnano 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
      ...
      5)
        echo "1) 安装  2) 卸载"; read -p "请选择操作：" action
        case "$action" in
          1) $SUDO apt update && $SUDO apt install -y vim && echo -e "\e[32mvim 安装完成！\e[0m" || echo -e "\e[31mvim 安装失败！\e[0m" ;;
          2) $SUDO apt remove -y vim && echo -e "\e[32mvim 卸载完成！\e[0m" || echo -e "\e[31mvim 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
      ...
      6)
        echo "1) 安装  2) 卸载"; read -p "请选择操作：" action
        case "$action" in
          1) $SUDO apt update && $SUDO apt install -y zip && echo -e "\e[32mzip 安装完成！\e[0m" || echo -e "\e[31mzip 安装失败！\e[0m" ;;
          2) $SUDO apt remove -y zip && echo -e "\e[32mzip 卸载完成！\e[0m" || echo -e "\e[31mzip 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
      ...
      7)
        echo "1) 安装  2) 卸载"; read -p "请选择操作：" action
        case "$action" in
          1) $SUDO apt update && $SUDO apt install -y git && echo -e "\e[32mgit 安装完成！\e[0m" || echo -e "\e[31mgit 安装失败！\e[0m" ;;
          2) $SUDO apt remove -y git && echo -e "\e[32mgit 卸载完成！\e[0m" || echo -e "\e[31mgit 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
      ...
      8)
        echo "1) 安装  2) 卸载"; read -p "请选择操作：" action
        case "$action" in
          1) $SUDO apt update && $SUDO apt install -y htop && echo -e "\e[32mhtop 安装完成！\e[0m" || echo -e "\e[31mhtop 安装失败！\e[0m" ;;
          2) $SUDO apt remove -y htop && echo -e "\e[32mhtop 卸载完成！\e[0m" || echo -e "\e[31mhtop 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
      ...
      9)
        echo "1) 安装  2) 卸载"; read -p "请选择操作：" action
        case "$action" in
          1) if curl -fsSL https://get.docker.com/ | $SUDO sh; then echo -e "\e[32mdocker 安装完成！\e[0m"; else echo -e "\e[31mdocker 安装失败！\e[0m"; fi ;;
          2) $SUDO apt remove -y docker && echo -e "\e[32mdocker 卸载完成！\e[0m" || echo -e "\e[31mdocker 卸载失败！\e[0m" ;;
          "") ;;
          *) echo -e "\e[31m无效选项。\e[0m" ;;
        esac
        pause
  ...
  while true; do
    ...
    case "$cert_choice" in
      1)
        ...
        $SUDO apt update
        command -v crontab >/dev/null 2>&1 || { echo "安装 cron..."; $SUDO apt install -y cron || echo -e "\e[31mcron 安装失败！\e[0m"; }
        command -v socat   >/dev/null 2>&1 || { echo "安装 socat..."; $SUDO apt install -y socat || echo -e "\e[31msocat 安装失败！\e[0m"; }
      ...
      2)
        ...
        if ~/.acme.sh/acme.sh --issue --standalone -d "$domain"; then
          ...
          echo -e "\e[31m证书申请失败，请检查域名解析与放行80端口。\e[0m"
        fi
        pause
        ;;
      3)
        ...
        echo -e "\e[31m切换失败，请确认 acme.sh 安装与网络。\e[0m"
        pause
        ;;
      4)
        read -p "请输入域名: " domain
...
install_xray_tls() {
  while true; do
    ...
    case "$xray_choice" in
      1)
        if bash -c "$(curl -fsSL https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install && \
        ...
        ;;
      2)
        echo -e "\e[33m提示：将 UUID 等参数填入 /usr/local/etc/xray/config.json；若已用“申请证书”安装证书，证书路径可不改。\e[0m"
        command -v nano >/dev/null 2>&1 || { $SUDO apt update >/dev/null 2>&1; $SUDO apt install -y nano >/dev/null 2>&1 || { echo -e "\e[31m无法安装 nano！\e[0m"; pause; return; }; }
      ...
      3)
        ...
        get_domain_from_cert() { openssl x509 -in "$1" -text -noout | grep -aPo "DNS:[^,]*" | sed 's/DNS://' | head -n1 || openssl x509 -in "$1" -text -noout | grep -aPo "CN=[^ ]*" | sed 's/CN=//'; }
        get_public_ip() { curl -s https://api.ipify.org || curl -s -6 https://api64.ipify.org || echo "127.0.0.1"; }
        ...
        if [[ -z "$CERT_PATH" ]]; then echo -e "\e[31m未能找到证书路径。\e[0m"; pause; return; fi
  ...
  while true; do
    ...
    case "$xray_choice" in
      1)
        ...
        ;;
      2)
        echo -e "\e[33m提示：将 UUID、目标站与私钥填入配置文件（ShortIds 可留空）。\e[0m"
        command -v nano >/dev/null 2>&1 || { $SUDO apt update >/dev/null 2>&1; $SUDO apt install -y nano >/dev/null 2>&1 || { echo -e "\e[31m无法安装 nano！\e[0m"; pause; return; }; }
        $SUDO nano /usr/local/etc/xray/config.json
        pause
        ;;
      3)
        ...
        extract_list_field() {
          local list_parent=$1 list_field=$2
          if [[ "$list_field" == "shortIds" || "$list_field" == "serverNames" ]]; then
            local r
            r=$(grep -aA 2 "\"$list_field\": \[" "$CONFIG_PATH" | awk 'NR==2{gsub(/^\s+|\s*\/\/.*$/,"");split($0,a,","); for(i in a){gsub(/^["\s]+|["\s]+$/,"",a[i]);printf "%s ",a[i]}}')
            [[ -n "$r" ]] && remove_spaces_and_quotes "$r"
          else
            grep -aPoz "\"$list_parent\":\s*\[\s*\{[^}]*\}\s*\]" "$CONFIG_PATH" | grep -aPo "\"$list_field\":\s*\"[^\"]*\"" | head -n1 | sed -E "s/\"$list_field\":\s*\"([^\"]*)\"/\1/"
          fi
        }
        get_public_ip() { curl -s https://api.ipify.org || curl -s -6 https://api64.ipify.org || echo "127.0.0.1"; }
        ...
        SID=${SHORT_IDS:-""}
        PBK=${PUBLIC_KEY}   # 在“安装升级”步骤已导出的公钥
  ...
  while true; do
    ...
    case "$hysteria_choice" in
      1)
        ...
        ;;
      2)
        echo -e "\e[33m提示：将域名等参数填入 /etc/hysteria/config.yaml。\e[0m"
        command -v nano >/dev/null 2>&1 || { $SUDO apt update >/dev/null 2>&1; $SUDO apt install -y nano >/dev/null 2>&1 || { echo -e "\e[31m无法安装 nano！\e[0m"; pause; return; }; }
      ...
      3)
        ...
        get_domain_from_cert() { openssl x509 -in "$1" -text -noout | grep -Po "DNS:[^,]*" | head -n1 | sed 's/DNS://' || openssl x509 -in "$1" -text -noout | grep -Po "CN=[^ ]*" | sed 's/CN=//'; }
        get_ip_address() {
          local ip=""
          ip=$(curl -4 -s --connect-timeout 5 https://ifconfig.me 2>/dev/null)
          if [[ -n "$ip" && "$ip" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then echo "$ip"; return; fi
          ip=$(curl -6 -s --connect-timeout 5 https://ifconfig.me 2>/dev/null)
          if [[ -n "$ip" && "$ip" =~ ^[0-9a-fA-F:]+$ ]]; then echo "[$ip]"; return; fi
          echo ""; return 1
        }
        if [ ! -f "$config_file" ]; then echo -e "\e[31m未能找到配置文件。\e[0m"; pause; return; fi
        ...
        if ! systemctl is-active --quiet hysteria-server.service; then
          ...
          echo -e "\e[32mhysteria 已启动！\e[0m"
        fi

        local port password domain cert_path ip
        port=$(awk -F: '/^listen:/{print $3}' "$config_file"); port=${port:-443}
        ...
        ip=$(get_ip_address)
        if [ -z "$ip" ]; then echo -e "\e[31m无法获取IP地址。\e[0m"; pause; return; fi
      ...
      4)
        ...
        local config_file="/etc/hysteria/config.yaml"
        local redirect_port
        redirect_port=$(awk -F: '/listen:/{print $NF}' "$config_file")
        ...
        [[ "$start_port" =~ ^[0-9]+$ && "$start_port" -ge 1 && "$start_port" -le 65535 ]] || { echo -e "\e[31m起始端口无效，使用 60000\e[0m"; start_port="$default_start_port"; }
        read -p "请输入结束端口号 (默认 65535): " end_port; end_port=${end_port:-$default_end_port}
        [[ "$end_port" =~ ^[0-9]+$ && "$end_port" -ge "$start_port" && "$end_port" -le 65535 ]] || { echo -e "\e[31m结束端口无效，使用 65535\e[0m"; end_port="$default_end_port"; }
        mapfile -t interfaces < <(ip -o link | awk -F': ' '$2!="lo"{print $2}')
        [[ ${#interfaces[@]} -eq 0 ]] && { echo -e "\e[31m未找到网络接口。\e[0m"; pause; return; }
    ...
    case "$singbox_choice" in
      1)
        if bash <(curl -fsSL https://sing-box.app/deb-install.sh) && \
           $SUDO curl -fsSL -o /etc/sing-box/config.json "https://raw.githubusercontent.com/sezhai/VPS-Script/refs/heads/main/extras/sing-box/config.json"; then
        ...
        ;;
      2)
        echo -e "\e[33m提示：根据提示修改 /etc/sing-box/config.json。\e[0m"
        command -v nano >/dev/null 2>&1 || { $SUDO apt update >/dev/null 2>&1; $SUDO apt install -y nano >/dev/null 2>&1 || { echo -e "\e[31m无法安装 nano！\e[0m"; pause; return; }; }
      ...
      3)
        ...
        if ! systemctl is-active --quiet sing-box; then
          ...
          echo -e "\e[32msing-box 已启动！\e[0m"
        fi

        get_ip() {
          local ipv4 ipv6
          ipv4=$(curl -s -4 --connect-timeout 5 https://api.ipify.org 2>/dev/null)
          [[ -z "$ipv4" ]] && ipv4=$(curl -s -4 --connect-timeout 5 https://icanhazip.com 2>/dev/null | tr -d '\n')
          if [[ "$ipv4" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then echo "$ipv4"; return 0; fi
          ipv6=$(curl -s -6 --connect-timeout 5 https://api6.ipify.org 2>/dev/null)
          [[ -z "$ipv6" ]] && ipv6=$(curl -s -6 --connect-timeout 5 https://icanhazip.com 2>/dev/null | tr -d '\n')
          [[ -n "$ipv6" && "$ipv6" =~ ^[0-9a-fA-F:]+$ ]] && { echo "$ipv6"; return 0; }
          echo "127.0.0.1"
        }
        ...
        if grep -q '"tag":\s*"vmess"' "$CONFIG_PATH"; then
          ...
          vmess_port=$(grep -A 5  '"tag":\s*"vmess"' "$CONFIG_PATH" | grep -o '"listen_port":\s*[0-9]*' | awk -F: '{print $2}' | tr -d ' ,')
        ...
        if grep -q '"tag":\s*"reality"' "$CONFIG_PATH"; then
          ...
          vless_port=$(grep -A 5  '"tag":\s*"reality"' "$CONFIG_PATH" | grep -o '"listen_port":\s*[0-9]*' | awk -F: '{print $2}' | tr -d ' ,')
        ...
        if grep -q '"tag":\s*"hysteria2"' "$CONFIG_PATH"; then
          ...
          h2_port=$(grep -A 5  '"tag":\s*"hysteria2"' "$CONFIG_PATH" | grep -o '"listen_port":\s*[0-9]*' | awk -F: '{print $2}' | tr -d ' ,')
          ...
          if [ -n "$h2_pass" ] && [ -n "$h2_port" ] && [ -f "$cert_path" ]; then
            ...
            [ -n "$h2_domain" ] && { echo "hysteria2 链接如下："; echo -e "\e[34mhysteria2://$(urlencode "$h2_pass")@$ip_for_url:$h2_port?sni=$h2_domain&insecure=0#hysteria2\e[0m"; }
          fi
        fi
        pause
        ;;
      4)
        if $SUDO systemctl disable --now sing-box && $SUDO rm -f /usr/local/bin/sing-box /etc/systemd/system/sing-box.service && $SUDO rm -rf /var/lib/sing-box /etc/sing-box; then
...
install_1panel() {
  while true; do
    ...
    case "$panel_choice" in
      ...
      3) $SUDO apt install -y ufw && echo -e "\e[32mufw 安装完成！\e[0m" || echo -e "\e[31mufw 安装失败！\e[0m"; pause ;;
      4) $SUDO apt remove -y ufw && $SUDO apt purge -y ufw && $SUDO apt autoremove -y && echo -e "\e[32mufw 卸载完成。\e[0m" || echo -e "\e[31mufw 卸载失败！\e[0m"; pause ;;
      5)
...
while true; do
  ...
  if [[ -z "$choice" ]]; then
    echo -e "\e[32m退出脚本，感谢使用！\e[0m"; exit 0
  fi
  case "$choice" in
    1) view_vps_info ;;
    ...
    8) install_sing-box ;;
    9) install_1panel ;;
    *) echo -e "\e[31m无效选项，请输入 1-9 或直接回车退出！\e[0m" ;;
  esac
done
